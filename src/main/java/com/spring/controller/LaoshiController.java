 ...
 * 老师 */
@Controller
public class LaoshiController extends BaseController
{
    @Autowired
    private LaoshiMapper dao;
    @Autowired
    private LaoshiService service;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/laoshi_list")
    public String list()
    {
        ...
        Example example = new Example(Laoshi.class); //  创建一个扩展搜索类
        ...
        List<Laoshi> list = service.selectPageExample(example , page , pagesize);   // 获取当前页的行数
    ...
    @RequestMapping("/laoshi_updt")
    public String updt()
    {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Laoshi mmm = service.find(id);
        assign("mmm" , mmm);
        assign("updtself" , 0);
    ...
    @RequestMapping("/laoshi_updtself")
     ...
     * @return
     */
    @RequestMapping("/laoshiinsert")
    public String insert()
    {
        _var = new LinkedHashMap(); // 重置数据
        String tmp="";
        Laoshi post = new Laoshi();  // 创建实体类
        // 设置前台提交上来的数据到实体类中
    ...
    * @return
    */
    @RequestMapping("/laoshiupdate")
    public String update()
    {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Laoshi post = new Laoshi();
        // 将前台表单数据填充到实体类
        if(!Request.get("gonghao").equals(""))
        post.setGonghao(Request.get("gonghao"));
        ...
        int charuid = post.getId().intValue();
            ...
            return showSuccess("保存成功" , "laoshi_updtself.do");
        }
    ...
    */
    ...
    public String delete()
    {
        ...
        HashMap map = Query.make("laoshi").find(id);
